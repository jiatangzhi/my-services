...
def setup_logging(level=log.INFO, log_file: str | None = None):
    ...
    # optional: lower noise of third party libs 
...
class CreditScoringModelTraining:
    ...
    def _load_and_split_data(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        if 'Unnamed: 0' in df.columns:
            ...
            log.info("✔ Column 'Unnamed: 0' eliminada del DataFrame.")
    ...
    def _compute_metrics(y_true: np.ndarray, y_prob: np.ndarray, threshold: float = 0.5) -> Dict[str, float]:
        """
        Calculate accuracy, precision, recall, f1 y ROC-AUC.
        y_true: (N,)
    ...
    def _log_model_with_signature(self, model: nn.Module, x_example: torch.Tensor):
        """
        Log the model with signature and input_example. Try using 'name' (new),
        if not available in your version, try fallback to 'artifact_path'.
        Also add pip_requirements to skip the torch warning +cuXXX.
        """
        ...
        pip_requirements = [
            # If you use rueda CUDA local, you can add the PyTorch index:
            # "-f https://download.pytorch.org/whl/cu126",
            # "torch==2.7.1+cu126",
            "torch==2.7.1",  # generic fallback for PyPI
            "scikit-learn",
            "pandas",
            "numpy",
        ]
    ...
    def _log_plots_and_reports(self, y_true_val: np.ndarray, y_prob_val: np.ndarray):
        ...
        # ROC / PR y Confusion Matrix (validation)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="config/training/training_config-german_credit_risk_v110.yaml",
...
"""
execute training:
python src/training/train.py --config config/training/training_config-german_credit_risk_v100.yaml
python src/training/train.py --config config/training/training_config-german_credit_risk_v110.yaml
python src/training/train.py --config config/training/training_config-german_credit_risk_v120.yaml
python src/training/train.py --config config/training/training_config-german_credit_risk_v130.yaml
"""
