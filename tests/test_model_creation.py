...
def test_model_architecture(model_config_fixture):
    """
    Validate the network architecture (layers, dimensions, order) is built correctly.
    """
    log.info("TEST: Verifying the neural network architecture.")
    model = CreditScoringModel(**model_config_fixture)
    net = model.network
    
    # The expected architecture is: (Linear -> BatchNorm -> ReLU -> Dropout) -> (Linear -> BatchNorm -> ReLU -> Dropout) -> Linear
    expected_layers_count = len(model_config_fixture["hidden_layers"]) * 4 + 1
    assert len(net) == expected_layers_count, f"Expecting {expected_layers_count} layers, but {len(net)} are found."
